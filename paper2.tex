...
\begin{figure}
  ...
  \caption{Model improvement resulting from using smaller mass tiles
    ...
    SpaghettiLens, the red line being the ensemble mean.  Locations of
    images (maximum, saddle point, minimum) are marked with vertical
    arrows.  Crossing the horizontal $\kappa=1$ line is the effective
    Einstein radius, and is so labelled. The upper panel is from
    ...
    innermost 9 tiles each with 9 smaller tiles.}
...
for the 21 innermost mass tiles \todo{Check meaning of hires
  parameters.} solves the problem completely, but doubles the
