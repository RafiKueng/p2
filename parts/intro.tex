...
discovered through Space~Warps.  Purpose is twofold.  First, see if
plausible mass models can produce the overall configuration.  Second,
interpretation as lensing galaxies.

Each lens candidates is modelled in a collaborative refinement
process, where anyone interested can creata a new model or modify an
...
\item First we have diagnostics based on morphology of the
  system.\todo{Make ordering consistent in text and
    Table~\ref{tab:models}.}  Section~\ref{sec:morph} and
  Figure~\ref{fig:splinput} explain.
\begin{itemize}
...
\item Whether the arrival-time surface and synthetic image are
  ...
  which are not seen.  Figures~\ref{fig:arriv} and \ref{fig:synth} and
  \ref{fig:encl}.
\end{itemize}
\item Third, whether the implied lensing mass is plausible, given the
  ...
  extrapolate the stellar masses to a halo mass using the
