...
\caption{Total mass in the model against the estimated stellar mass,
  ...
  $0<\haloindex<1$.
  The width of the bars represent the error in $\Mstel$, where the
  lower and upper bounds are given assuming a ``young'' / ``old'' age trend
  for the population synthesis models (see Section~\ref{sec:stellar-mass}).
  The height of the bar is given by the minima and maxima values in $\Mhalo$
  obtained from the statistical ensemble of mass maps that make up
  one {\SpL} model (see Section~\ref{sec:massmodels}).
  \label{fig:stelmass}}
\end{figure*}
