...
\caption{Nine of the lens candidates marked up with spaghetti
  ...
  appearance has no significance.  These images are screenshots from the 
  ...
  best-modelled systems, but also one case (\sw{57} at upper right) of
...
\caption{Arrival-time surfaces for models of the systems from
  Fig.~\ref{fig:splinput}.  The registration differs slightly from
  Fig.~\ref{fig:splinput}, but the coloured dots represent exactly
...
\caption{Synthetic images of the systems from
  Fig.~\ref{fig:splinput}, derived from the lens models.  The
  reconstructed lensed features keep the \SW false-colour
  scheme from Fig~\ref{fig:splinput}.  The rest has been changed
  to black and white.
  \label{fig:synth}}
\end{figure*}
...
\caption{Ensemble-average mass distribution $\kappa$ for which the
  results Figs~\ref{fig:arriv}--\ref{fig:synth} were derived.  The
  dashed curves denote $\kappa=1$.  Most of the mass maps have a
  \SI{180}{\degree}-rotation symmetry, which is imposed by default.  For
  \sw{02} and \sw{57}, where the lensing mass is clearly asymmetric, the
...
\caption{Cumulative circular-averages of the mass maps from
  Fig.~\ref{fig:kappa}, with uncertainties.  More precisely, we show
  ...
  vertical arrows mark the positions of the images (maxima, saddle
  points and minima).
  \label{fig:encl}}
\end{figure*}
...
\begin{figure*}
\includegraphics[width=\linewidth]{img/mlens_vs_mstel/mlens_vs_mstel}
\caption{Total mass in the model against the estimated stellar mass,
  ...
  Figs~\ref{fig:splinput}--\ref{fig:encl}.)  The horizontal extent
  of each bar indicates the extreme cases of a young (\SI{0.5}{Gyr}-old)
...
\begin{table*}
  \caption{Diagnostics of the selected models for each \SW
    candidate (see Section~\ref{sec:summary}).
    %A machine-readable version is available in the online supplement.
    }
  \label{tab:models}
  \include{parts/table}
\end{table*}
